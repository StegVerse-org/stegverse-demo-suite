...
def build_parser():
    ...
    for cmd in ("help","status","list","tree","bulk","receipts","reset","demo","explain"): sub.add_parser(cmd)
...
def print_help_screen():
    ...
    print("Commands: help, explain, status, list, tree, run <step_id>, get <document>, receipts, bulk, reset, demo")
    print("Examples:")
    print("  stegverse.cmd demo   (Windows)")
    print("  ./stegverse demo     (Unix-like shells)")

def explain_runtime(gate):
    ...
    print(f"receipts: {s['total_receipts']}\n")
...
def print_tree(gate):
    s = gate.describe_runtime()
    print("StegVerse"); print("|-- state:", s["current_state"]); print("|-- receipts:", s["total_receipts"]); print("|-- completed_steps")
...
def main():
    ...
    if args.command == "list":
        [print(f"{'UNLOCKED' if x['unlocked'] else 'LOCKED':8} {x['document']}  ({x['path']})") for x in gate.list_documents()]; return
    if args.command == "tree": print_tree(gate); return
    ...
    if args.command == "receipts":
        ...
        if not rec: print("No receipts yet."); return
        [print(f"{r['sequence']}. {r['step_id']} | {r['receipt_id']} | prev={r['previous_receipt_id']}") for r in rec]; return
    if args.command == "reset": gate.reset(); print("Runtime reset complete."); return
    if args.command == "run":
    ...
    if args.command == "get":
        ...
        return
    if args.command == "explain": explain_runtime(gate); return
