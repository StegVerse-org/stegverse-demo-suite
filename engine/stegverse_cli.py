...
def build_parser() -> argparse.ArgumentParser:
    ...
    for cmd in ("help", "status", "list", "tree", "bulk", "receipts", "action-receipts", "actions", "reset", "demo", "explain"):
        sub.add_parser(cmd)
...
def explain_runtime(gate: StegVerseGate, action_gate: ActionGate) -> None:
    ...
    print("\ngoverned actions:")
    for row in action_gate.list_actions():
        print(f" - {row['action']} (requires {row['required_state']})")
    print("\nunlocked artifacts:")
...
def print_tree(gate: StegVerseGate, action_gate: ActionGate) -> None:
    s = gate.describe_runtime()
    print("StegVerse")
    ...
    if s["completed_steps"]:
        for x in s["completed_steps"]:
            print("|   |--", x)
    else:
        print("|   `-- none")
    print("|-- next_admissible_steps")
    if s["next_admissible_steps"]:
        for x in s["next_admissible_steps"]:
            print("|   |--", x)
    else:
        print("|   `-- none")
...
def main() -> None:
    ...
    if args.command == "list":
        for x in gate.list_documents():
            print(f"{'UNLOCKED' if x['unlocked'] else 'LOCKED':8} {x['document']}  ({x['path']})")
        return
    if args.command == "tree":
        print_tree(gate, action_gate)
        return
    ...
    if args.command == "receipts":
        ...
        if not rec:
            print("No workflow receipts yet.")
            return
        for r in rec:
            print(f"{r['sequence']}. {r['step_id']} | {r['receipt_id']} | prev={r['previous_receipt_id']}")
        return
    if args.command == "action-receipts":
        rec = action_gate.action_receipt_chain()
        if not rec:
            print("No action receipts yet.")
            return
        for r in rec:
            print(
                f"{r['sequence']}. {r['action_name']} | {r['action_receipt_id']} | "
                f"prev={r['previous_action_receipt_id']} | decision={r['decision']}"
            )
        return
    if args.command == "actions":
        for row in action_gate.list_actions():
            print(f"{row['action']:15} requires={row['required_state']}  {row['description']}")
        return
    ...
    if args.command == "explain":
        explain_runtime(gate, action_gate)
        return
