...
class ActionReceipt:
    ...
    action_name:str
    sequence:int
    previous_action_receipt_id:str
    current_state:str
    required_state:str
    decision:str
    timestamp_utc:str
...
class ActionGate:
    def __init__(self, repo_root:Path)->None:
        ...
        self.actions_path=self.repo_root/'engine'/'actions'/'actions.json'
        self.runtime_dir=self.repo_root/'.stegverse_runtime'
        self.action_receipts_path=self.runtime_dir/'action_receipts.json'
        self.runtime_dir.mkdir(parents=True, exist_ok=True)
        self.actions_config=json.loads(self.actions_path.read_text(encoding='utf-8'))
        self.gate=StegVerseGate(self.repo_root)
    ...
    def request_action(self, action_name:str):
        ...
        receipt=ActionReceipt(rid,action_name,len(receipts)+1,prev,current_state,required_state,decision,ts,digest); receipts.append(receipt.to_dict()); self._save_action_receipts(receipts)
