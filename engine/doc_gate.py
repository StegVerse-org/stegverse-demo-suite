...
class StegVerseGate:
    ...
    def reset(self):
        self._save_state({'current_state':self.initial_state,'completed_steps':[],'unlocked_documents':self._compute_unlocked_documents([])})
        self._save_receipts([])
    def _compute_unlocked_documents(self, completed_steps:List[str])->List[str]:
        completed=set(completed_steps)
        return sorted([d for d,rule in self.doc_rules.items() if set(rule.get('required_steps',[])).issubset(completed)])
    def status(self):
        s=self._load_state()
    ...
    def run_step(self, step_id:str):
        ...
        state['current_state']=next_state; state['completed_steps']=completed; state['unlocked_documents']=unlocked; receipts.append(receipt.to_dict())
        self._save_state(state); self._save_receipts(receipts)
