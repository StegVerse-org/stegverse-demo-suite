...
class StegVerseGate:
    ...
    def _save_receipts(self, receipts: List[Dict[str, object]]) -> None:
        self.receipts_path.write_text(json.dumps(receipts, indent=2), encoding="utf-8")
    ...
    def _compute_unlocked_documents(self, completed_steps: List[str]) -> List[str]:
        ...
        return sorted([
            doc for doc, rule in self.doc_rules.items()
            if set(rule.get("required_steps", [])).issubset(completed)
        ])
    ...
    def status(self) -> Dict[str, object]:
        ...
        return {
            ...
            "unlocked_documents": s["unlocked_documents"],
    ...
    def next_admissible_steps(self) -> List[str]:
        ...
        return sorted([
            ...
            if step_id not in completed and step["from_state"] == current
        ])
    ...
    def describe_runtime(self) -> Dict[str, object]:
        ...
        return {
            ...
            "locked_documents": [
                d for d in sorted(self.doc_rules)
                if d not in set(st["unlocked_documents"])
            ],
        }
    ...
    def list_documents(self) -> List[Dict[str, object]]:
        ...
        return [
            {"document": d, "path": f"docs/{d}", "unlocked": d in unlocked}
            for d in sorted(self.doc_rules)
        ]
    ...
    def run_step(self, step_id: str) -> Dict[str, object]:
        ...
        if current != step["from_state"]:
            raise ValueError(
                f"Step {step_id} is not admissible from state {current}. "
                f"Expected {step['from_state']}."
            )
        ...
        unlocked = self._compute_unlocked_documents(completed)
        ...
        state["unlocked_documents"] = unlocked
